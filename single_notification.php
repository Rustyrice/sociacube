...
<?php
  ...
  if(is_array($actor) && is_array($owner)){
    ...
    if($actor['userid'] != $id){
      //echo "<a href='". ROOT . "$actor[type]/$actor[tag_name]' style='font-weight:bold;text-decoration:none;color:black;'>".$actor['first_name'] . ' ' . $actor['last_name']."</a>";
    ...
    $time = Time::get_time($notif_row['date']);
    echo "<br>";
    echo "<span style='font-size:11px;color:#888;display:inline-block;margin-right:10px;'>$time</span>";
