...
class User
{
	...
	public function get_friends($id)
	{
		...
		$query = "select * from users where userid != '$id' && type= 'profile' order by rand() limit 2";
		$DB = new Database();
		$result = $DB->read($query);
	...
	public function get_mentions($id, $tags)
	{

		$query = "select * from posts where userid != '$id' && tags = '$tags' order by id desc limit 2";
		$DB = new Database();
		$result = $DB->read($query);
