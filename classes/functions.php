...
function add_notification($userid, $activity, $row, $tagged_user = '')
{
	...
	$query = "select * from notifications where contentid = '$contentid' && userid = '$userid' && activity = '$activity' limit 1";
	$check = $DB->read($query);
...
function check_notifications()
{
	...
	if (count($follow) > 0) {
		...
		//$query = "select * from notifications where (userid != '$userid' && content_owner = '$userid') || (userid != '$userid') order by id desc limit 30";
	} else {
		...
		foreach ($words as $word) {
			...
			if (preg_match("/@[a-zA-Z_0-9\Q.?!,'\E]+/", $word)) {
				...
				if (is_array($user_row)) {
					$user_row = $user_row[0];
					$str .= "<a href='" . ROOT . "profile/$user_row[tag_name]' id='name_tag'>@" . $word . "</a> ";
				} else {
...
function group_access($userid, $group_data, $access)
{
	...
	if ($access == 'member') {
		...
		$query = "select userid from group_members where groupid = '$groupid' && userid = '$userid' && (role = 'member' || role = 'admin' || role = 'moderator') && disabled = 0 limit 1";
		$data = $DB->read($query);
		if (is_array($data)) {
			return true;
		}
...
function i_follow($userid)
{
	$user = new User();
	$friends = $user->get_following($_SESSION['mybook_userid'], "user");
	if (is_array($friends)) {
		$userids = array_column($friends, 'userid');
