	...
	<a href="<?=ROOT?>profile/<?php echo $FRIEND_ROW['userid']; ?>" class="username">
		...
		<?php
			...
			if($invited && isset($INVITER_ROW)){
				...
				echo "<div style='font-size:10px;color:gray;'>You were invited by " . $INVITER_ROW['first_name'] . " " . $INVITER_ROW['last_name'] . "</div><br>";
			}
		?>
		<a href="<?=ROOT?>group_request_accept/<?=$group_data['userid']?>/<?=$FRIEND_ROW['userid']?>/decline">
			<input id="post_button" type="button" value="Decline" style="float:left;background-color:#916e1b;width:auto;font-size:12px;">
		</a>
		<a href="<?=ROOT?>group_request_accept/<?=$group_data['userid']?>/<?=$FRIEND_ROW['userid']?>/accept">
			<input id="post_button" type="button" value="Accept" style="float:right;background-color:#1b9186;width:auto;font-size:12px;">
		</a>
