	...
	public function create_post($userid, $data, $files, $owner = 0)
	{
		...
		if (!empty($data['post']) || !empty($files['file']['name']) || isset($data['is_profile_image']) || isset($data['is_cover_image'])) {
			...
			if (isset($data['is_profile_image']) || isset($data['is_cover_image'])) {
				...
				if (!empty($files['file']['name'])) {
					...
					if (in_array($files['file']['type'], $allowed)) {
						...
						$this->error .= "The selected file is not a valid type<br>";
					}
				}
			}
			...
			if ($this->error == "") {
				...
				$query = "insert into posts (owner,userid,postid,post,image,has_image,is_profile_image,is_cover_image,parent,tags,date) values ('$owner','$userid','$postid','$post','$myimage','$has_image','$is_profile_image','$is_cover_image','$parent','$tags','$date')";
				$DB->save($query);
